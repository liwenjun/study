# 实战`Ubuntu-22.04-LTS`配置开发环境(精简版)
...
# 创建启动脚本 wsl2.bat
# 以管理员身份运行：
wsl --mount --vhd C:\wsl\workspace.vhd --bare
```
...
/dev/sdc1: UUID="e6d326e2-4b38-4956-8ca5-a4861e021023"

# 挂载
echo "UUID=e6d326e2-4b38-4956-8ca5-a4861e021023 /mnt/workspace ext4 defaults 0 0" | sudo tee -a /etc/fstab
```
...
# 安装
sudo apt install nano unzip tree pkg-config
#
...
#
mkdir -p /mnt/workspace/pgsql+data
sudo chown postgres:postgres /mnt/workspace/pgsql+data

# 迁移文件
sudo -u postgres mv /var/lib/postgresql/17 /mnt/workspace/pgsql+data/

# 创建链接
cd /var/lib/postgresql
sudo -u postgres ln -s /mnt/workspace/pgsql+data/17
```
...
cd ~
ln -s /mnt/workspace/dev+base/Android
ln -s /mnt/workspace/dev+base/.android
...
cd /mnt/workspace/dev+base
sudo tar -zxvf /mnt/z/android-studio-2024.2.1.13-linux.tar.gz
cd /opt
sudo ln -s /mnt/workspace/dev+base/android-studio
