...
impl From<ReservedErrorCode> for (i64, &'static str) {
    fn from(error_code: ReservedErrorCode) -> Self {
        match error_code {
            ReservedErrorCode::ParseError => (error_code as i64, "解析错误"),
            ReservedErrorCode::InvalidRequest => (error_code as i64, "无效请求"),
            ReservedErrorCode::MethodNotFound => (error_code as i64, "无此方法"),
            ReservedErrorCode::InvalidParams => (error_code as i64, "无效参数"),
            ReservedErrorCode::InternalError => (error_code as i64, "内部错误"),
        }
    }
}
...
mod tests {
    ...
    fn should_construct_reserved_error() {
        const EXPECTED_WITH_DATA: &str =
            r#"{"code":-32700,"message":"解析错误","data":{"id":1314,"context":"TEST"}}"#;
        const EXPECTED_WITHOUT_DATA: &str = r#"{"code":-32601,"message":"无此方法"}"#;
        const EXPECTED_WITH_BAD_DATA: &str = r#"{"code":-32603,"message":"内部错误","data":"failed to json-encode additional info in json-rpc error: won't encode"}"#;
    ...
    fn should_fall_back_to_internal_error_on_bad_custom_error() {
        const EXPECTED: &str = r#"{"code":-32603,"message":"内部错误","data":"attempted to return reserved error code -32603"}"#;
