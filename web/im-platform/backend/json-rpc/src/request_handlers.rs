...
type RequestHandler =
    Arc<dyn Fn(Option<Params>, Option<String>) -> HandleRequestFuture + Send + Sync>;
...
impl RequestHandlers {
    ...
    pub(crate) async fn handle_request(&self, request: Request) -> Response {
        ...
        match handler(request.params, request.auth).await {
...
impl RequestHandlersBuilder {
    ...
    /// ```ignore
    /// async fn handle_it(params: Option<Params>, auth: Option<String>) -> Result<T, Error>
    /// ```
    ...
    where
        Func: Fn(Option<Params>, Option<String>) -> Fut + Send + Sync + 'static,
        ...
        T: Serialize + 'static,
    {
        ...
        let wrapped_handler = move |maybe_params, maybe_auth| {
            let handler = Arc::clone(&handler);
            async move {
                let success = Arc::clone(&handler)(maybe_params, maybe_auth).await?;
                serde_json::to_value(success).map_err(|error: serde_json::Error| {
