...
const AUTH_FIELD_NAME: &str = "auth";
    ...
    pub(super) fn new(
        ...
        match request.remove(JSONRPC_FIELD_NAME) {
            Some(Value::String(jsonrpc)) => {
                if jsonrpc != JSON_RPC_VERSION {
                    let error = Error::new(
                        ReservedErrorCode::InvalidRequest,
                        format!("'jsonrpc' 应该是字符串 '2.0', 但获取值是 '{}'", jsonrpc),
                    );
            ...
            None => {
                let error = Error::new(
                    ReservedErrorCode::InvalidRequest,
                    format!("缺失 '{}' 字段", JSONRPC_FIELD_NAME),
                );
                return Err(ErrorOrRejection::Error { id, error });
            }
        }
        ...
        let method = match request.remove(METHOD_FIELD_NAME) {
            ...
            Some(_) => {
                let error = Error::new(
                    ReservedErrorCode::InvalidRequest,
                    format!("期望 '{}' 是字符串", METHOD_FIELD_NAME),
                );
                return Err(ErrorOrRejection::Error { id, error });
            }
            None => {
                let error = Error::new(
                    ReservedErrorCode::InvalidRequest,
                    format!("缺失 '{}' 字段", METHOD_FIELD_NAME),
                );
                return Err(ErrorOrRejection::Error { id, error });
            }
        };

        let auth = match request.remove(AUTH_FIELD_NAME) {
            Some(Value::String(auth)) => Some(auth),
            Some(Value::Null) => None,
            Some(_) => {
                let error = Error::new(
                    ReservedErrorCode::InvalidRequest,
                    format!("期望 '{}' 是字符串或空值", AUTH_FIELD_NAME),
                );
                return Err(ErrorOrRejection::Error { id, error });
            }
            None => None,
        ...
        Ok(Request { id, method, params, auth })
    }
}
...
mod tests {
    ...
    fn should_fail_to_validate_with_missing_jsonrpc_field() {
        ...
        assert_eq!(
            error,
            Error::new(ReservedErrorCode::InvalidRequest, "缺失 'jsonrpc' 字段")
        );
    }
    ...
    fn should_fail_to_validate_with_missing_method_field() {
        ...
        assert_eq!(
            error,
            Error::new(ReservedErrorCode::InvalidRequest, "缺失 'method' 字段")
        );
    }
