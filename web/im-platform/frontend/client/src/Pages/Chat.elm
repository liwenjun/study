...
update user shared msg model =
    case msg of
        ...
        ClickLeaveGuild guild ->
            --( { model | canLeaveGuild = True }, Effect.none )
            ( model
            , ChatApi.Guilds.leave
                { url = shared.baseUrl
                , token = user.token.token |> Just
                , params = guild
                , handler = HandleLeaveGuild
                }
                |> Effect.sendCmd
            )

        HandleLeaveGuild data ->
            ( model, Effect.none )
...
view user shared model =
    { title = "交流平台"
    , body = [ Chat.view user shared model ]
    }
